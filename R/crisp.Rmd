...
for the second case study presented in the accompanying 
...
sample names. We used the software [kallisto](https://doi.org/10.1038/nbt.3519) to 
quantify counts from the RNA-seq data (with 100 bootstrap replicates). The 
`path` column contains the path to the kallisto abundance file for each sample.
...
with a log-fold change greater than 2, at least 10 counts, and a $p$-value less than 0.01:
```{r filtered-results}
...
```
We demonstrate use of these data for manual plotting in the [human embryo case study](https://raw.githack.com/l-a-yates/cpam_manuscript/main/R/torre.html).
