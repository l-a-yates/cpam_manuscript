---
title: "`cpam`: an introductory example"
date: "`r Sys.Date()`"
...
omics data. It serves as a basic introduction to the package. There are also 
two detailed case studies using real world data:

 - [Arabisdopsis Case Study](https://raw.githack.com/l-a-yates/cpam_manuscript/main/R/torre.html)
...
These data are gene-level counts from a case-only experiment with 6 time points and 5 replicates per time point.
The code to reproduce the data is available in this repository
`R/simulate_counts_example.R`. 
...
## Experimental design
The experimental design for this example loads is included in the `cpam` package. 
Simply type the folowing to see the design.
...
estimate changepoints, and select the shape for each gene 
In this simple example, the simulated data are gene-level, that is we do not 
have isoform-level counts. As such, we leave NULL the transcript-to-gene mapping
...
with a log-fold change greater than 2, at least 10 counts, and a $p$-value less than 0.01, we can run
```{r filtered-results}
...
Let's look for a gene that has a more complex trend. Unconstrained shapes in `cpam` are denoted by `'tp'`. We can filter the results for genes with unconstrained shapes and plot one of them.
```{r find-tp-gene}
results(cpo) %>% 
  filter(shape == "tp")
```
